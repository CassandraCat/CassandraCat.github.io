...
我自己最近做了一次工作流的二次自我审计，按四个角度（反哺机制端到端 / skill 重复 / Wiki schema 漂移 / MCP 死工具）跑下来，11 条 finding 里有 7 条真问题、2 条误报、2 条已经修过了。在这件事上我形成了一个比较清晰的三段成熟度模型：

- **第一段：运行**——工具体系能跑起来。Skill 写完了，MCP 接好了，反哺路径连通了。能用，但还在依赖人主动触发。
- **第二段：自我演进**——能审计自己 + 修复自己。能发现"哪里漂移了"、"哪些功能重复了"、"哪些工具建了没人用"。这一段需要**人触发 + 人 review + 人 verify**——尤其是 agent 自动跑出来的 audit 报告，~30% 是误报，盲改会修出新坑，必须逐条 verify。
...
我现在卡在第二段——能审计能修但成本高（一次 audit 大概一两小时，修法靠手工）。第三段是接下来要做的方向，但不急，先让第二段的频率稳下来（每修完一波大功能跑一次），再去自动化值得自动化的部分。

一个反直觉的结论：**能演化的工具集才活着**。一开始我以为工作流的成熟度看"功能多不多 / 链路全不全"，做久了发现这是错的——功能多但漂移没人管的体系会迅速腐烂。真正决定能不能用三年五年的，是**演化机制本身的成熟度**。这一层在地图上看不到（地图画的是当前结构），但读者跟着这套思路搭自己工具的时候，请在 day 1 就给"周期 audit + 自动化漂移检测"留位置。
